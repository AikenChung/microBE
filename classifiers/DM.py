...
class DeepMicrobiome(object):
    def __init__(self, tr_data, ts_data, seed, data_dir):
        self.t_start = time.time()
        self.filename_train = str(tr_data)
        self.filename_test = str(ts_data)
        self.data_name = self.filename_train.split('_')[0]
        self.train_data = self.filename_train.split('.')[0]
        self.test_data = self.filename_test.split('.')[0]
        self.seed = seed
    ...
    def loadCustomData(self, dtype=None):
        ...
        # read train data
        filename_train = self.data_dir + "data/" + self.filename_train
        if os.path.isfile(filename_train):
            train = pd.read_csv(filename_train, sep=',', index_col=False, header=None)
        else:
            print("FileNotFoundError: Training file {} does not exist".format(filename_train))
            exit()
        
        # read test data
        filename_test = self.data_dir + "data/" + self.filename_test
        if os.path.isfile(filename_test):
            test = pd.read_csv(filename_test, sep=',', index_col=False, header=None)
        else:
            print("FileNotFoundError: Test file {} does not exist".format(filename_test))
            exit()

        # load data
        self.X_train = train.values.astype(dtype)
        self.X_test = test.values.astype(dtype)

        # put nothing or zeros for y_train, y_test, and X_test
        self.y_train = np.zeros(shape=(self.X_train.shape[0])).astype(dtype)
        self.y_test = np.zeros(shape=(self.X_test.shape[0])).astype(dtype)
        self.printDataShapes(train_only=True)

    def loadCustomDataWithLabels(self, trn_lbl_data, tst_lbl_data, dtype=None):
        
        # read train data and labels
        filename_train = self.data_dir + "data/" + self.filename_train
        trn_lbl_filename = self.data_dir + "data/" + trn_lbl_data
        if os.path.isfile(filename_train) and os.path.isfile(trn_lbl_filename):
            train_raw = pd.read_csv(filename_train, sep=',', index_col=False, header=None)
            train_label = pd.read_csv(trn_lbl_filename, sep=',', index_col=False, header=None)
        else:
            if not os.path.isfile(filename_train):
                print("FileNotFoundError: Train file {} does not exist".format(filename_train))
            if not os.path.isfile(trn_lbl_filename):
                print("FileNotFoundError: Train file {} does not exist".format(trn_lbl_filename))
            exit()
        
        # label data validity check
        if train_label.values.shape[1] > 1: 
            print('FileSpecificationError: The train_label file contains more than 1 column.')
            exit()
        # read test data and labels
        filename_test = self.data_dir + "data/" + self.filename_test
        tst_lbl_filename = self.data_dir + "data/" + tst_lbl_data
        if os.path.isfile(filename_test) and os.path.isfile(tst_lbl_filename):
            test_raw = pd.read_csv(filename_test, sep=',', index_col=False, header=None)
            test_label = pd.read_csv(tst_lbl_filename, sep=',', index_col=False, header=None)
        else:
            if not os.path.isfile(filename_test):
                print("FileNotFoundError: Test file {} does not exist".format(filename_test))
            if not os.path.isfile(tst_lbl_filename):
                print("FileNotFoundError: Test file {} does not exist".format(tst_lbl_filename))
            exit()
    ...
    def ae(self, dims = [50], epochs= 2000, batch_size=100, verbose=2, loss='mean_squared_error', latent_act=False, output_act=False, act='relu', patience=20, val_rate=0.1, no_trn=False):
        ...
        modelName = self.prefix + self.data_name + '.h5'
    ...
    def vae(self, dims = [10], epochs=2000, batch_size=100, verbose=2, loss='mse', output_act=False, act='relu', patience=25, beta=1.0, warmup=True, warmup_rate=0.01, val_rate=0.1, no_trn=False):
    ...
    def cae(self, dims = [32], epochs=2000, batch_size=100, verbose=2, loss='mse', output_act=False, act='relu', patience=25, val_rate=0.1, rf_rate = 0.1, st_rate = 0.25, no_trn=False):
    ...
    def classification(self, hyper_parameters, method='svm', cv=5, scoring='roc_auc', n_jobs=1, cache_size=10000):
        ...
        with open(self.data_dir + "results/" + self.data_name + "_result.txt", 'a') as f:
            res.to_csv(f, header=None)

        print('Accuracy metrics')
        print('AUC, ACC, Recall, Precision, F1_score, MCC, time-end, runtime(sec), classfication time(sec), best hyper-parameter')
        print(metrics)
    ...
    def saveLossProgress(self):
        ...
        figureName = self.prefix + self.data_name + '_' + str(self.seed)
        ...
        if 'recon_loss' in self.history.history:
            figureName = self.prefix + self.data_name + '_' + str(self.seed) + '_detailed'
            plt.ylim(min(loss_collector) * 0.9, loss_max_atTheEnd * 2.0)
            plt.plot(self.history.history['loss'])
            plt.plot(self.history.history['val_loss'])
...
if __name__ == '__main__':
    ...
    load_data.add_argument("-cdtrn", "--custom_train_data", help="filename for custom training data under the 'data' folder", type=str,)
    load_data.add_argument("-cltrn", "--custom_train_labels", help="filename for custom training labels under the 'data' folder", type=str,)
    load_data.add_argument("-cdtst", "--custom_test_data", help="filename for custom test data under the 'data' folder", type=str,)
    load_data.add_argument("-cltst", "--custom_test_labels", help="filename for custom test labels under the 'data' folder", type=str,)
    ...
    def run_exp(seed):
        ...
        ## no argument founded
        if args.custom_train_data != None and args.custom_test_data != None :

            ### without labels - only conducting representation learning
            if args.custom_train_labels == None or args.custom_test_labels == None :
                dm = DeepMicrobiome(tr_data=args.custom_train_data,
                                    ts_data=args.custom_test_data,
                                    seed=seed, data_dir=args.data_dir)
            ...
            ### with labels - conducting representation learning + classification
                ...
                dm = DeepMicrobiome(tr_data=args.custom_train_data,
                                    ts_data=args.custom_test_data,
                                    seed=seed, data_dir=args.data_dir)
                dm.loadCustomDataWithLabels(trn_lbl_data=args.custom_train_labels,  
                                            tst_lbl_data=args.custom_test_labels,
                                            dtype=dtypeDict[args.dataType])
        
        else :
            print("[Error] Please specify an input file(s). (use -h option for help)")
            exit()
        ...
        if args.save_rep:
            if numRLrequired == 1:
                rep_file = dm.data_dir + "results/" + dm.prefix + dm.data_name + "_rep.csv"
